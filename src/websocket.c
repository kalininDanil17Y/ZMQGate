...
static void heartbeat(struct ev_loop *loop,
    ...
    if(zmq_msg_send(&msg, route->websocket.forward._sock,
    ...
    if(zmq_msg_send(&msg, route->websocket.forward._sock, ZMQ_DONTWAIT) < 0) {
        if(errno != EAGAIN) { //TODO: EINTR???
            SNIMPL(-1);
        } // else: nevermind
        zmq_msg_close(&msg);
...
static void send_sync(struct ev_loop *loop,
    ...
    zmq_msg_t msg;
    SNIMPL(zmq_msg_init_data(&msg, root.instance_id, IID_LEN, NULL, NULL));
    if(zmq_msg_send(&msg, socket->_sock, ZMQ_SNDMORE|ZMQ_DONTWAIT) < 0) {
        if(errno != EAGAIN) { //TODO: EINTR???
            SNIMPL(-1);
        } else {
            zmq_msg_close(&msg);
    ...
    LIST_FOREACH(item, &socket->_int.outputs, output_list) {
        ...
        prev = item;
        SNIMPL(zmq_msg_send(&msg, socket->_sock, ZMQ_DONTWAIT|ZMQ_SNDMORE));
    ...
    SNIMPL(zmq_msg_send(&msg, socket->_sock, ZMQ_DONTWAIT));
}
